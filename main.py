...
def upload_to_drive(local_path, folder_id, drive):
...
def scrape_top_pins(query: str, limit: int = 50):
    ...
    # wait for top pins to load
    time.sleep(7)

    # grab first N unique pin links
    ...
    for link in pin_links:
        try:
            driver.get(link)
            time.sleep(3)  # allow pin page to load
