...
def is_valid_image(image_data, min_size_kb=50):
    image = Image.open(BytesIO(image_data))
    width, height = image.size
    size_kb = len(image_data) / 1024  # in KB
    print(f"Checking image size: {size_kb:.2f} KB, Resolution: {width}x{height}")
    # Portrait check (height > width) + minimum dimensions + file size
    return height > width and width >= 500 and height >= 800 and size_kb >= min_size_kb
...
def download_duplicates_file(service, folder_id):
    ...
    if items:
        ...
        print("Downloaded existing downloaded_urls.txt from Drive.")
        return file_id
    else:
        print("No previous downloaded_urls.txt found on Drive.")
        return None
...
def main():
    ...
    # Fetch or create duplicates tracking file
        ...
        for idx, url in enumerate(image_urls):
            ...
            try:
                ...
                if not is_valid_image(img_data):
                    print(f"Skipping low-quality or non-portrait image: {url}")
                    continue
    ...
    # Save and upload updated duplicates file
